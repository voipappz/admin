...
export class JanusStore extends ComponentStore<VideoroomState> implements OnDestroy {
  ...
  readonly attachAudioRoom = this.effect((url$: Observable<string>) => {
    return url$.pipe(
      switchMap((url: string) => {
        this.log('attach', url);
        this.reduce(new actions.AttachAudioRoom(url));
        return this.janusService.attachAudioRoom(url)
          .pipe(
            tap((data: JanusAttachCallbackData) => {
              this.reduce(new actions.AttachCallback(data));
            }),
            catchError((error) => {
              return of(this.reduce(new actions.AttachAudioRoomFail(error)));
            })
          );
      })
    );
