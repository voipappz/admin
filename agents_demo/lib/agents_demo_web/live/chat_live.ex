defmodule AgentsDemoWeb.ChatLive do
  ...
  defp post_operator_reply(socket, text) do
    case Conversations.post_human_reply(socket.assigns.current_scope, socket.assigns.conversation_id, text) do
      {:ok, _message} ->
  ...
  # Every surface enters through `AgentsDemo.Turns`, which applies the handler
  ...
  defp subscribe_to_conversation(socket, conversation_id) when is_binary(conversation_id) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(AgentsDemo.PubSub, Conversations.topic(conversation_id))
    end
