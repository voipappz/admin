defmodule ConnectixWeb.Portal.EventControllerTest do
  ...
  `config :agents_demo, :events_store` — so nothing here depends on whatever
  ...
  setup do
    ...
    Application.put_env(:agents_demo, :events_store, store)
    ...
    on_exit(fn ->
      Application.delete_env(:agents_demo, :events_store)
      File.rm(path)
    end)
