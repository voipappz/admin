defmodule Connectix.SkillsTest do
  ...
  test "fetch/2 resolves a compatible version and refuses the rest" do
    assert {:ok, Connectix.Skills.WebLookup} = Skills.fetch("web_lookup", "1.0.0")
    assert {:ok, Connectix.Skills.WebLookup} = Skills.fetch("web_lookup", "1.4.2")
  ...
  test "validate_settings/3 types settings through the skill's schema" do
    assert {:ok, %Connectix.Skills.MemoryFiles.Settings{enabled_tools: ["read_file"]}} =
    ...
    assert {:ok, %Connectix.Skills.Todo.Settings{inline: true}} =
