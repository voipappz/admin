...
top of the Elixir LangChain library. The `agents_demo` application shows how
...
Configuration is environment variables, and `AgentsDemo.Config` is the one place
...
`AgentsDemo.Config.summary/0` prints the resolved set with secrets reduced to
...
- **`Mix` does not exist in a release.** `Mix.env()` at runtime crashes in
  production; `AgentsDemo.Config` reads it once at compile time instead, which
...
Adding one is a module implementing `AgentsDemo.Channels.Channel` plus a line in
`config :agents_demo, :channels`. `AgentsDemo.Channels.deliver/2` is the single
dispatch point — resist adding per-channel calls at call sites.

Delivery hangs off `AgentsDemo.Agents.DisplayMessagePersistence.save_message/3`.
...
**The webhook is mounted in `AgentsDemoWeb.Endpoint`, not the router, and ahead
...
- `AgentsDemo.Bots.Bot` is identity only: name, slug, description, status,
  and two pointers — `current_version` (published; what new conversations
  pin) and `draft_version` (being edited, if any).
- `AgentsDemo.Bots.BotVersion` is the whole definition: eleven typed
  embedded areas (`AgentsDemo.Bots.Version.*`) plus `bot_version_skills`
...
- Publishing (`Bots.publish_draft/3`) validates through
  `AgentsDemo.Bots.Validator` (one report shared with the API's preflight),
  fingerprints the canonical content (`AgentsDemo.Bots.Snapshot`), and moves
...
- **Skills** are code in a literal registry (`AgentsDemo.Skills`): id →
  module implementing `AgentsDemo.Skills.Skill` with a settings schema,
  ...
  had.
- **Capabilities** (`AgentsDemo.Skills.Capability` +
  `AgentsDemo.Capabilities.Capability`) carry their policy (risk, approval,
...
- **Compiler** (`AgentsDemo.Bots.Compiler` → `CompiledSpec`): pure and
...
AgentsDemo.Agents.Factory. New work must move those choices into a validated,
...
Use this pipeline:
    ...
    Pipecat browser client
      ...
      -> speech-to-text
      -> AgentsDemo.Voice.SagentsBridge
...
AgentsDemo.Voice.SagentsBridge is the only boundary between voice frames and
...
AgentsDemo.Channels.Channel. Channels.deliver/2 handles completed asynchronous
messages. The active voice socket consumes Sagents deltas for low latency, while
AgentsDemo.Agents.DisplayMessagePersistence.save_message/3 remains the
...
behind the AgentsDemo.Voice boundary so upstream changes do not leak across the
...
- **`assets.deploy` must compile first.** Colocated LiveView hooks are emitted
  ...
  resolve `phoenix-colocated/agents_demo` — in dev the directory is left over
