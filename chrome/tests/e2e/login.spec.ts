...
// The login form no longer asks for a domain — the extension talks to one
// host, `CONFIG.API_ENDPOINT`. `_domain` is its runtime override, and seeding
// it is how a spec aims the extension somewhere other than that default.
const useDomain = (page, domain) =>
...
test.describe('Login page', () => {
  test('renders username and password fields, and no domain field', async ({ popupPage }) => {
    // Asserted absent, not merely unused: the endpoint is configuration now,
    // and a field left on screen would invite someone to type a host that
    // nothing reads.
    await expect(popupPage.locator('input[formcontrolname="domain"]')).toHaveCount(0);
  ...
  test('invalid credentials show error snackbar', async ({ popupPage }) => {
    ...
    await useDomain(popupPage, DOMAIN);
