...
// CallInfo shape: { id, direction: "inbound"|"outbound", remote,
//   state: "ringing"|"connecting"|"active"|"ended", connectedAt?, failureReason? }
// CallStateEvent: { type: "start", call } | { type: "ringing"|"connecting"|"established"|"terminated", id, at? } | { type: "clear", id }
...
export function reduceCallState(current, event) {
...
export function pruneEndedSessions(sessions, activeId) {
...
// SipCredentials shape: { username, password, domain?, displayName? }
// SipLogEntry shape: { ts, level, category, content }
...
function attachRemoteStream(pc, audioEl, onBlocked) {
...
function setupRemoteAudio(session, audioEl, onBlocked) {
...
const peerConnectionOf = (session) => session?.sessionDescriptionHandler?.peerConnection;
...
function applyTrackState(session, held, muted) {
...
async function reinviteHoldWithRetry(session, hold, attempts = 6) {
...
function reinviteHold(session, hold) {
  return new Promise((resolve, reject) => {
    ...
    // `hold` belongs to the WEB SessionDescriptionHandler's options.
    const sdhOptions = {
...
export function useSipPhone(overrides = {}) {
  ...
  const [transfer, setTransferInfo] = useState(null);
  const [consult, setConsultInfo] = useState(null);
  const [lastError, setLastError] = useState(null);
  const [networkAvailable, setNetworkAvailable] = useState(() => typeof navigator === 'undefined' || navigator.onLine !== false);

  const uaRef = useRef(null);
  const regRef = useRef(null);
  const sessionsRef = useRef(new Map());  // keyed by ctxid (uuid)
  const invitationRef = useRef(null);     // current ringing inbound
  const answeringRef = useRef(false);     // guards double-click accept
  const hangupRequestedRef = useRef(new Set()); // hangup while accept is settling
  const activeIdRef = useRef(null);       // ctxSip.callActiveID
  ...
  const cfgRef = useRef(loadSipConfig(overrides));
  ...
  const addLog = useCallback((level, category, content) => {
  ...
  const reportError = useCallback((category, error, fallback) => {
  ...
  const wireSession = useCallback((session, info) => {
    session.ctxid = info.id;
    ...
    session.stateChange.addListener((state) => {
      ...
      if (state === SessionState.Established) {
        ...
        if (invitationRef.current && invitationRef.current.ctxid === info.id) invitationRef.current = null;
        ...
        if (hangupRequestedRef.current.delete(info.id)) void session.bye().catch(() => {});
  ...
  }, [dispatchTransfer, addLog, reportError]);
  ...
  const buildUA = useCallback(async () => {
    ...
    try {
      ua = new UserAgent({
        ...
        logConnector: (level, category, _label, content) => addLog(level, category, content),
        sessionDescriptionHandlerFactoryOptions: { peerConnectionConfiguration: { iceServers: cfg.iceServers } },
        delegate: { onInvite: (invitation) => handleIncomingCall(invitation) }
      });
    } catch (err) {
    ...
    ua.transport.stateChange.addListener((tstate) => {
  ...
  const dial = useCallback(async (target, cfgOverrides = {}) => {
  ...
  }, [overrides, wireSession, status, reportError, addLog]);
  ...
  const setHeld = useCallback(async (value) => {
    const id = activeIdRef.current;
    const session = id ? sessionsRef.current.get(id) : null;
  ...
  const referOptions = useCallback((kind, target) => {
  ...
  const transferBlind = useCallback(async (target) => {
   ...
   */
  const startAttendedTransfer = useCallback(async (target) => {
    const ua = uaRef.current;
    establishedCall();
  ...
  const setDoNotDisturb = useCallback((value) => {
