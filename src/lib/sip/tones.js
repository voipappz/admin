...
// TonePattern shape: { freqs: number[], onMs: number, offMs: number, gain: number (0..1) }

/** Inbound: pulsing, insistent, meant to be noticed across a room. */
export const RINGTONE = { freqs: [440, 480], onMs: 700, offMs: 700, gain: 0.06 };

/** Outbound after 180: ITU cadence (425 Hz, 1s on / 3s off) — "it is ringing at the far end". */
export const RINGBACK = { freqs: [425], onMs: 1000, offMs: 3000, gain: 0.045 };
 ...
 */
export function startTone(pattern) {
  const ctx = context();
  if (!ctx) return () => {};
...
export function resetToneAudioForTests() {
  unhook?.();
  shared = null;
}
