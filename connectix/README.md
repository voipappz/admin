...
![Connectix Files Interface](screenshots/Connectix-Files.png)
...
![Connectix Todo Interface](screenshots/Connectix-Todo.png)

*The agent-managed TODO list displayed in the sidebar, tracking tasks across the conversation.*

![Connectix HITL Interface](screenshots/Connectix-HITL.png)
...
**Note**: The core integration files (`lib/connectix/agents/coordinator.ex`, `lib/connectix/agents/factory.ex`, and `lib/connectix_web/live/agent_live_helpers.ex`) are generated by the `mix sagents.setup` task. This task creates the boilerplate needed to integrate the Sagents framework into your Phoenix application.
...
#### 1. Coordinator (`lib/connectix/agents/coordinator.ex`)
...
#### 2. Factory (`lib/connectix/agents/factory.ex`)
...
#### 3. Conversations Context (`lib/connectix/conversations.ex`)
...
#### 4. ChatLive (`lib/connectix_web/live/chat_live.ex`)
...
#### 5. AgentLiveHelpers (`lib/connectix_web/live/agent_live_helpers.ex`)
...
- [`lib/connectix/agents/coordinator.ex`](lib/connectix/agents/coordinator.ex) - Lifecycle management
- [`lib/connectix/agents/factory.ex`](lib/connectix/agents/factory.ex) - Agent configuration

**Database Layer:**
- [`lib/connectix/conversations.ex`](lib/connectix/conversations.ex) - Conversation context
- [`lib/connectix/conversations/conversation.ex`](lib/connectix/conversations/conversation.ex) - Schema
- [`lib/connectix/conversations/agent_state.ex`](lib/connectix/conversations/agent_state.ex) - State schema
- [`lib/connectix/conversations/display_message.ex`](lib/connectix/conversations/display_message.ex) - Message schema

**Phoenix LiveView:**
- [`lib/connectix_web/live/chat_live.ex`](lib/connectix_web/live/chat_live.ex) - Main chat interface
- [`lib/connectix_web/live/agent_live_helpers.ex`](lib/connectix_web/live/agent_live_helpers.ex) - LiveView state management helpers (Generated by `mix sagents.setup`)

**Custom Middleware:**
- [`lib/connectix/middleware/web_tool_middleware.ex`](lib/connectix/middleware/web_tool_middleware.ex) - Web search integration (example of custom middleware)

### Customization Guide

#### Change Agent Configuration

Edit the generated [`lib/connectix/agents/factory.ex`](lib/connectix/agents/factory.ex:1) to add/remove middleware or change models:
...
Edit the generated [`lib/connectix/agents/coordinator.ex`](lib/connectix/agents/coordinator.ex:1):
...
1. Add new content_type to [`display_messages`](lib/connectix/conversations/display_message.ex:1) schema
2. Add handler in [`Conversations`](lib/connectix/conversations.ex:1) context:
   ```elixir
   def append_custom_message(conversation_id, data) do
     # Create DisplayMessage with content_type: "custom"
   end
   ```
3. Update [`ChatLive`](lib/connectix_web/live/chat_live.ex:1) to handle new type
...
The web search tool serves as a **practical example of creating custom middleware** for the Sagents framework. The implementation in [`lib/connectix/middleware/web_tool_middleware.ex`](lib/connectix/middleware/web_tool_middleware.ex) demonstrates:
