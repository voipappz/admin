...
other than the form you just saved. Nothing here needs a developer, a terminal
...
- **Result** — `Pass`, `Fail`, `Blocked` (something else broke first) or `Not Run`.
- A `Fail` needs a **Notes** entry: what you saw instead, and the time, so the
...
## Before you start (all blocks)

- An account with full access on the environment under test, and its password.
- The sign-in code (OTP) arrives by email. On a test environment the mail is not
  delivered to a real inbox — ask whoever set the environment up where it lands.
- A softphone (Zoiper, Linphone, a desk phone) for the rows that make or take a
  real call.
...
## MY ACCOUNT AND CUSTOMER  (`/account`)

| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-19 | Your own profile | You are signed in. | 1. Open your account.<br>2. Change your display name and save.<br>3. Reload. | The new name is kept and is shown in the top bar. | MEDIUM | | | Not Run | |
| T-20 | Company details | You have full access. | 1. Open the customer details.<br>2. Change the company contact details and save.<br>3. Reload. | The change is kept. | MEDIUM | | | Not Run | |
| T-21 | Switching customer | needs: an account with more than one customer. | 1. Use the customer switcher.<br>2. Open Devices and Calls. | Every screen now shows the other customer's data, and nothing of the first. | HIGH | | | Not Run | |
| T-22 | Changing your password | You know your current password. | 1. Change your password.<br>2. Sign out and back in. | Only the new password works. | MEDIUM | | | Not Run | |
...
| T-27 | Bulk change | Several applications. | 1. Select more than one.<br>2. Apply a change to all of them. | Every selected application is changed; the others are untouched. | LOW | | | Not Run | |
...
| T-40 | Import devices from a file | A CSV of a few devices. | 1. Press Import CSV.<br>2. Choose the file and import. | Every row becomes a device and the count matches. | MEDIUM | | | Not Run | |
| T-41 | Filter the list | More than a page of devices. | 1. Use the filters and the search box.<br>2. Page through. | Results match what you asked for; paging does not lose or repeat rows. | LOW | | | Not Run | |
| T-42 | Delete a device | The device from T-34. | 1. Delete it.<br>2. Try to register its softphone. | It is gone from the list and can no longer register. | HIGH | | | Not Run | |

---

## ROUTING AND NUMBERS  (`/routing`)
...
| T-48 | Copy a number's setup | A number that is fully set up. | 1. Press Duplicate DID.<br>2. Give it another number and save. | The copy has the same routing, with the new number. | LOW | | | Not Run | |
| T-49 | Import numbers from a file | A CSV of numbers. | 1. Import DIDs from CSV. | Every row becomes a number and the count matches. | MEDIUM | | | Not Run | |
| T-50 | Delete a number | The number from T-43. | 1. Delete it.<br>2. Call it. | It is gone from the list and the call is not answered by the platform. | HIGH | | | Not Run | |
| T-51 | Calling between two sites | needs: two PBXs / sites connected by a trunk rule. | 1. From a phone on site A, dial a site B extension. | The call reaches site B and connects. | MEDIUM | | | Not Run | |

---

## ROUTING RESOURCES — queues, IVRs, announcements, conditions

| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-52 | Create a queue | A device to act as agent. | 1. Press Create Queue.<br>2. Add the device as an agent.<br>3. Save. | The queue is listed with its agent. | HIGH | | | Not Run | |
| T-53 | A queue call reaches the agent | The queue from T-52, agent registered. | 1. Call into the queue. | The agent's phone rings; answering connects the caller; the call appears in Calls. | HIGH | | | Not Run | |
| T-54 | Agent availability | The queue from T-52. | 1. Set the agent unavailable.<br>2. Call the queue.<br>3. Set them available and call again. | An unavailable agent is not rung; an available one is. | MEDIUM | | | Not Run | |
| T-55 | Create an IVR | An announcement to play. | 1. Press Create IVR.<br>2. Add a greeting and at least one key option.<br>3. Save. | The IVR is listed with its options. | HIGH | | | Not Run | |
| T-56 | Upload an announcement | An audio file. | 1. Press Create New Announcement.<br>2. Upload the file and save.<br>3. Play it back on the screen. | The recording is stored and plays back correctly. | MEDIUM | | | Not Run | |
| T-57 | Make an announcement from text | Text-to-speech configured. | 1. Open an announcement.<br>2. Type the words and press Generate Audio.<br>3. Play it back. | The spoken audio matches the text and is saved. | MEDIUM | | | Not Run | |
| T-58 | Create a call condition | An application exists. | 1. Press Create Call Condition.<br>2. Set a time range and a destination.<br>3. Save. | It is listed and can be picked when routing a number. | MEDIUM | | | Not Run | |
| T-59 | Voicemail | A device with voicemail. | 1. Call it and leave a message when it goes unanswered.<br>2. Open the voicemail list. | The message is listed and plays back. | MEDIUM | | | Not Run | |
...
| T-69 | Add a node | You are root. | 1. Add a node and save. | It appears in the list. | MEDIUM | | | Not Run | |
| T-70 | Only root sees Nodes | An account without node rights. | 1. Sign in as it.<br>2. Look for Nodes, then type the address directly. | The screen is neither listed nor reachable. | HIGH | | | Not Run | |

---

## CALLS  (`/calls`, `/calls-log`)
...
| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-80 | An incoming message arrives | needs: a messaging channel set up. | 1. Send a message to the platform number from outside. | It appears in Messages against the right conversation. | MEDIUM | | | Not Run | |
| T-81 | Reply to a message | The conversation from T-80. | 1. Reply from the screen. | The reply reaches the sender and is shown in the thread. | MEDIUM | | | Not Run | |
...
| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-82 | Create a campaign | A number to call from. | 1. Create a campaign and add numbers to call. | It is listed with its numbers. | MEDIUM | | | Not Run | |
| T-83 | Run a campaign | The campaign from T-82. needs: a number you can answer. | 1. Start it. | The numbers are dialled, answered calls follow the campaign's flow, and each attempt shows in the call log. | MEDIUM | | | Not Run | |
| T-84 | Stop a campaign | A campaign running. | 1. Stop it. | No further numbers are dialled. | MEDIUM | | | Not Run | |
...
| T-90 | Screen pop / CRM | needs: a CRM account. | 1. Set up the screen-pop service.<br>2. Take a call from a number known to the CRM. | The record opens for the agent. | MEDIUM | | | Not Run | |
| T-91 | Delete a service | The service from T-85. | 1. Delete it.<br>2. Wait a minute and make a call. | It is gone from the list and nothing more is sent. | MEDIUM | | | Not Run | |

---

## SUBSCRIPTIONS, TARIFFS AND BILLING
...
| T-99 | What a number costs | The tariff from T-98, with rates for `1` and `1212`. | 1. Ask the tariff for the rate for a +1 212 number. | The more specific rate (`1212`) is the one quoted, not `1`. | HIGH | | | Not Run | |
| T-100 | A disabled rate is not used | The tariff from T-98. | 1. Disable one rate.<br>2. Ask for a number it covered. | That rate is no longer quoted. | MEDIUM | | | Not Run | |
| T-101 | Create a plan | A tariff. | 1. Create a plan and put the tariff on it.<br>2. Save. | It can be picked when adding a subscription. | MEDIUM | | | Not Run | |
| T-102 | Transactions add up | A few charged calls. | 1. Open Transactions and compare with the call log for the same period. | The charges match the calls; nothing is missing or doubled. | MEDIUM | | | Not Run | |

---

## AUTOMATION — workflows, bots, schemas, templates

| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-103 | Create a workflow | You have full access. | 1. Press New Workflow.<br>2. Build a small flow and save. | It is listed and can be reopened exactly as saved. | MEDIUM | | | Not Run | |
| T-104 | A workflow runs on a call | The workflow from T-103, attached to a call flow. | 1. Make a call that triggers it.<br>2. Open Events for that call. | The workflow's steps are recorded against the call. | MEDIUM | | | Not Run | |
| T-105 | Create a bot | needs: an AI provider configured. | 1. Press Create Bot.<br>2. Give it instructions and save. | It is listed. | MEDIUM | | | Not Run | |
| T-106 | A bot answers a call | The bot from T-105, pointed at a number. | 1. Call the number.<br>2. Speak to it. | It answers, responds sensibly, and the call is in the call log. | MEDIUM | | | Not Run | |
| T-107 | Create a schema | You have full access. | 1. Press New Schema.<br>2. Define it and save. | It is listed and reopens as saved. | LOW | | | Not Run | |
| T-108 | Create a template | You have full access. | 1. Create a template and save.<br>2. Use it somewhere it is offered. | It is offered and applies what it defines. | LOW | | | Not Run | |
| T-109 | Ask the assistant | AI configured. | 1. Open the assistant and ask about your own data (for example how many calls today). | The answer matches what the screens show. | LOW | | | Not Run | |
...
## MONITORING, HEALTH, LOGS AND EVENTS
...
| T-118 | Recent activity in Logs | Anything done in the last few minutes. | 1. Open Logs. | Recent lines are there, newest first, with times that make sense. | HIGH | | | Not Run | |
| T-119 | Find a line | A call you just made. | 1. Search Logs for its number or reference. | The lines for that call are found. | HIGH | | | Not Run | |
| T-120 | Filter by app and severity | Some traffic. | 1. Filter to errors only, then to one app. | Only matching lines are listed. | MEDIUM | | | Not Run | |
...
| ID | What we are testing | Before you start | What to do | What you should see | Priority | Tester | Date | Result | Notes |
|---|---|---|---|---|---|---|---|---|---|
| T-125 | Run a report | Some calls in the period. | 1. Open Reports, pick a period, run one. | The figures agree with the call log for that period. | HIGH | | | Not Run | |
| T-126 | Export a report | The report from T-125. | 1. Export it. | The file downloads and has the same rows as the screen. | MEDIUM | | | Not Run | |
| T-127 | Notifications | Something that raises one. | 1. Open Notifications.<br>2. Mark one as read. | It is listed, and it stays read after a reload. | LOW | | | Not Run | |
